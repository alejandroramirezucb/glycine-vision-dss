...
|---|---|---|---|
| Recall de hoja del segmentador | 0.977 | `05_evaluacion_modelos` | `mseg_test_metrics.json` → `leaf_recall` |
| Dice del segmentador | 0.885 | `05_evaluacion_modelos` | `mseg_test_metrics.json` → `dice` |
| IoU del segmentador | 0.808 | `05_evaluacion_modelos` | `mseg_test_metrics.json` → `iou` |
...
| Ablación por componente, media ± DE, 3 semillas | Tabla 5 | `11_ablacion_presupuesto_completo` | `ablation_full_m2.csv` |
...
| Clasificación por píxel: sano, clorótico, necrótico, defoliado | `leaf_analyzer.py`, umbrales sobre L\*, a\* y b\* |
...
- La validación experta empleó **60 hojas y un solo evaluador**, sin acuerdo inter-evaluador. La verdad de campo del patógeno proviene de la etiqueta del conjunto de datos, no de diagnóstico de laboratorio.
